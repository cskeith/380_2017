<%@ page import="edu.ouhk.comps380f.lab05.PageVisit" %>
<!DOCTYPE html>
<html>
    <head>
    ...
    <body>
        ...
        <c:forEach var="visit" items="${activity}">
            ...
            <c:set target="${timeValues}" property="time" value="${visit.enteredTimestamp}" />
            (<fmt:formatDate value="${timeValues}" 
                        pattern="EEE, d MMM yyyy HH:mm:ss Z" />
            <c:if test="${!empty visit.leftTimestamp}">, 
                stayed for ${PageVisit.toString(visit.leftTimestamp - visit.enteredTimestamp)}
            </c:if>)
            <br />
        </c:forEach>
    </body>
